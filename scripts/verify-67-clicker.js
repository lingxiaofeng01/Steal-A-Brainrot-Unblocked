#!/usr/bin/env node
/**
 * Verification script for 67 Clicker game addition
 */

const fs = require('fs');
const path = require('path');

console.log('🔍 Verifying 67 Clicker Addition...\n');
...
if (gamesContent.includes("name: '67 Clicker'") &&
    gamesContent.includes("slug: '67-clicker'") &&
...
const pagePath = path.join(__dirname, '../src/pages/SixtySevenClickerPage.tsx');
if (fs.existsSync(pagePath)) {
  console.log('   ✅ SixtySevenClickerPage.tsx exists');
  const pageContent = fs.readFileSync(pagePath, 'utf-8');
  if (pageContent.includes("title: '67 Clicker'") &&
      pageContent.includes("slug: '67-clicker'")) {
    console.log('   ✅ Page component has correct game data');
    
    // Check keyword density (approximate)
    const matches = pageContent.match(/67 Clicker/g);
    const wordCount = pageContent.split(/\s+/).length;
    const density = ((matches ? matches.length : 0) / wordCount * 100).toFixed(2);
    console.log(`   ✅ Keyword "67 Clicker" appears ${matches ? matches.length : 0} times (${density}% density)`);
  ...
  console.log('   ❌ SixtySevenClickerPage.tsx NOT found');
  allChecks = false;
}
...
if (detailPageContent.includes("'67-clicker': lazy(() => import('./SixtySevenClickerPage'))")) {
...
const thumbnailPath = path.join(__dirname, '../public/images/thumbnails/67-clicker.webp');
...
if (sitemapContent.includes('<loc>https://www.stealabrainrotunblocked.com/67-clicker</loc>')) {
...
if (sitemapImagesContent.includes('<loc>https://www.stealabrainrotunblocked.com/67-clicker</loc>')) {
...
const relevantTags = ['clicker', 'idle', 'casual', 'meme', 'arcade'];
let tagsUpdated = 0;
relevantTags.forEach(tag => {
  if (sitemapTagsContent.includes(`<loc>https://www.stealabrainrotunblocked.com/tag/${tag}</loc>`) &&
      sitemapTagsContent.includes('<lastmod>2025-11-04</lastmod>')) {
    tagsUpdated++;
  }
});
console.log(`   ✅ ${tagsUpdated}/${relevantTags.length} relevant tag pages updated`);

// 8. Check Supabase migration script
console.log('\n8️⃣ Checking Supabase migration script...');
const sqlPath = path.join(__dirname, '../supabase/migrations/20251104_add_67_clicker.sql');
if (fs.existsSync(sqlPath)) {
  console.log('   ✅ 20251104_add_67_clicker.sql found');
  const sqlContent = fs.readFileSync(sqlPath, 'utf-8');
  
  if (sqlContent.includes("slug = '67-clicker'") || sqlContent.includes("'67-clicker'")) {
  ...
  console.log('   ❌ 20251104_add_67_clicker.sql NOT found');
  allChecks = false;
}
...
// 10. Check game tags
console.log('\n🔟 Checking game tags...');
const expectedTags = ['Clicker', 'Idle', 'Casual', 'Meme', 'Arcade', 'Incremental', 'Math'];
let tagsFound = 0;
...
console.log('\n' + '='.repeat(50));
if (allChecks) {
  console.log('✅ All checks passed! 67 Clicker is ready!');
  console.log('\n📋 Next steps:');
