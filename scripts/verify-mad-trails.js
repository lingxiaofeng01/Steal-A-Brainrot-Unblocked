...
const GAME_SLUG = 'mad-trails';
const GAME_NAME = 'Mad Trails';
const RELEASE_DATE = '2025-11-21';
const TAGS = ['Racing', 'Driving', 'Physics', 'Car', 'Action', 'Skill', '3D', 'Casual', 'Arcade', 'Unblocked', 'Browser', 'Adventure', 'Simulation', 'Climbing', 'Upgrade'];
...
const thumbnailPath = path.join(__dirname, '..', 'public', 'images', 'thumbnails', `${GAME_SLUG}.png`);
...
const pagePath = path.join(__dirname, '..', 'src', 'pages', 'MadTrailsPage.tsx');
if (fs.existsSync(pagePath)) {
  const pageContent = fs.readFileSync(pagePath, 'utf-8');
  console.log('   ✅ MadTrailsPage.tsx exists');
  
  // Check for key content
  if (pageContent.includes('Mad Trails')) {
  ...
  // Check content length (should be 1000+ words)
  const wordCount = pageContent.split(/\s+/).length;
  if (wordCount >= 1000) {
    console.log(`   ✅ Page has ${wordCount} words (1000+ requirement met)`);
  } else {
    console.log(`   ⚠️  Page has only ${wordCount} words (should be 1000+)`);
  }
} else {
  console.log('   ❌ MadTrailsPage.tsx not found');
  allChecksPassed = false;
}
...
if (gameDetailContent.includes(`'${GAME_SLUG}': lazy(() => import('./MadTrailsPage'))`)) {
...
const migrationPath = path.join(__dirname, '..', 'supabase', 'migrations', `20251121_add_mad_trails.sql`);
...
if (sitemapImagesContent.includes(`${GAME_SLUG}.png`)) {
...
const updatedTags = ['racing', 'driving', 'physics', 'car', 'action', 'skill', '3d', 'casual', 'arcade', 'browser', 'simulation', 'adventure'];
