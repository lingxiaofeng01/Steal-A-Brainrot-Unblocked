...
async function verifyDatabase() {
  console.log('🔍 Verifying Cheese Chompers 3D in Supabase...\n');
  ...
  try {
    ...
    const { data: commentsData, error: commentsError } = await supabase
      .from('game_comments')
      .select('*')
      .eq('game_slug', 'cheese-chompers-3d');
    ...
    console.log('\n🏷️  Checking tags...');
    const expectedTags = ['action', 'survival', 'fast-paced', 'arena', 'animal', '3d', 'multiplayer', 'physics'];
    ...
    if (allChecks) {
      ...
      console.log('🎮 Cheese Chompers 3D is ready in the database!');
      console.log('\n🚀 Next Steps:');
