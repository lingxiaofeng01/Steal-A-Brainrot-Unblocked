/**
 * Verification Script for Traffic Rally Game Addition
...
function analyzeSEO(filePath) {
  ...
  // Check for H1 tag
  const hasH1 = content.includes('Traffic Rally');
...
// Main verification
log('\n🔍 Traffic Rally Game Verification\n', 'cyan');

log('1️⃣  Checking Required Files:', 'yellow');
const filesExist = [
  checkFileExists('public/images/thumbnails/traffic-rally.png'),
  checkFileExists('src/pages/TrafficRallyPage.tsx'),
  checkFileExists('supabase/migrations/20251107_add_traffic_rally.sql'),
];

log('\n2️⃣  Checking File Contents:', 'yellow');
const contentsCorrect = [
  checkFileContains('src/data/games.ts', 'traffic-rally', 'Game data in games.ts'),
  checkFileContains('src/data/games.ts', 'Traffic Rally', 'Game name in games.ts'),
  checkFileContains('src/data/games.ts', '2025-11-07', 'Release date in games.ts'),
  checkFileContains('src/pages/GameDetailPage.tsx', 'traffic-rally', 'Route in GameDetailPage.tsx'),
  checkFileContains('src/pages/GameDetailPage.tsx', 'TrafficRallyPage', 'Component import in GameDetailPage.tsx'),
  checkFileContains('public/sitemap-games.xml', 'traffic-rally', 'Game in sitemap-games.xml'),
  checkFileContains('public/sitemap-images.xml', 'traffic-rally.png', 'Image in sitemap-images.xml'),
];

log('\n3️⃣  Checking Game Tags:', 'yellow');
const tags = ['Racing', 'Car', 'Driving', '3D', 'Action', 'Traffic', 'Highway', 'Speed'];
const tagsCorrect = tags.map(tag => 
  checkFileContains('src/data/games.ts', tag, `Tag: ${tag}`)
);

log('\n4️⃣  SEO Verification:', 'yellow');
analyzeSEO('src/pages/TrafficRallyPage.tsx');
...
if (passedChecks === totalChecks) {
  log('\n✅ All checks passed! Traffic Rally game is ready.', 'green');
  process.exit(0);
} else {
